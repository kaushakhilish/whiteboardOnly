...
const ShapeProperties = ({ setShapes, shape, rectProps, pdfOptionsObj }) => {
    ...
    return (
        <foreignObject x={rectProps?.x + shape?.props.translateX}
            y={rectProps?.y + shape?.props.translateY + rectProps.h + 10}
            width={rectProps.w > 150 ? rectProps.w : 200} height={60}>
            <div onClick={e => e.stopPropagation()} className={styles.shapeProperitesBox}>

                {(shape?.type !== SHAPES.PDF && shape?.type !== SHAPES.IMAGE)
                    && <>
                        <div className={styles.option}>
                            <label >
                                <div style={{ border: ('2px solid ' + shape?.style?.stroke) }} className={styles.strokeColor} />
                                <input onInput={strokeColorInputChangeHander} type="color" style={{ visibility: 'hidden', position: 'absolute' }} />
                            </label>
                        </div>
                        ...
                        <div className={styles.option}>
                            <label>
                                <div style={{ backgroundColor: shape?.style?.fill }} className={styles.fillColor}></div>
                                <input onInput={fillColorInputChangeHander} id='shapeSelectColor' type='color'
                                    style={{ visibility: 'hidden', position: 'absolute', border: '0px', width: '60%', height: '60%' }}
                        ...
                        <div style={{ width: '40px' }} className={styles.option}>
                            <input onInput={strokeWidthChangeHandler} min={1} defaultValue={shape?.style?.strokeWidth}
                                type="number" style={{ width: '100%', border: 'none', outline: 'none', paddingLeft: '2px', textAlign: 'center' }} />
                        </div>
                    </>}
